...
Image.MAX_IMAGE_PIXELS = None  # or a larger number than the default - The final crop won't work without this.
...
zoom_level = 1  # Set your desired zoom level (e.g., 19 for max resolution)
resolution_base = 0.1  # meters per pixel at zoom level 19
resolution = resolution_base * (2**(19 - zoom_level))  # Calculate resolution for the desired zoom level
...
num_workers = 32 # Set the number of worker threads
workers = []
...
# part 9: Crop the image to a square of size 8192x8192 with the original center
print("Cropping image to 8192px...")
start_time = time.time() # start timing
...
output_image_8192 = output_image.crop(crop_bbox)
output_image_path_8192 = os.path.join(args.output_location, 'Aerial_4km_8192px.png')
output_image_8192.save(output_image_path_8192)
end_time = time.time() # stop timing
print(f"Image cropping to 8192px took {1000 * (end_time - start_time):.2f} milliseconds.")


# part 10: Crop the image to a square of size 16384x16384 with the original center
print("Cropping image to 16384px...")
start_time = time.time() # start timing
half_size = 16384 / 2
crop_bbox = (center_x - half_size, center_y - half_size, center_x + half_size, center_y + half_size)
output_image_16384 = output_image.crop(crop_bbox)
output_image_path_16384 = os.path.join(args.output_location, 'Aerial_4096m_16384px.png')
output_image_16384.save(output_image_path_16384)
end_time = time.time() # stop timing
print(f"Image cropping to 16384px took {1000 * (end_time - start_time):.2f} milliseconds.")
